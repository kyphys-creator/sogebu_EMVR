...
public class BDirection2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Vector3 up1 = vp(bfield, new Vector3(0, 1, 0));
            // 方向を、回転情報に変換
            Quaternion brotation = Quaternion.LookRotation(up1, bfield);
            transform.rotation = brotation;
        }
        else
        {
            Vector3 up1 = vp(bfield, new Vector3(0, 0, 1));
            // 方向を、回転情報に変換
            Quaternion brotation = Quaternion.LookRotation(up1, bfield);
            transform.rotation = brotation;
        }
